...
- **文字对比度自动适配**（v20）：皮肤激活时按背景明暗自动切换应用文字主题——极光/单色预设（深色基底）自动启用深色文字主题（浅色文字+深色表面，解决浅色主题灰字看不清）；自定义图片按感知亮度采样自动选择深/浅文字主题。纯呈现层切换（`data-ds-dark-theme`），不改动设置里的外观偏好，关闭皮肤后精确还原；面板提示行显示当前适配状态。
