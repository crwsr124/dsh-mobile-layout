...
- **玻璃表面**（v7 兼容性重构）：表面半透明由 **JS 实时计算 rgba** 写入 token
  ...
  通透（×0.66）、输入框略实（×0.85）形成层次；**顶部栏固定 70% 不透明度**（不随滑杆变化），
  且以纵向渐变**淡出到全透明**（70%→0）——半透明层叠加在内容层之上，若渐变
  只到滑杆值会在接缝处双重叠加出白色带；淡到 0 后接缝处即内容区原值，完全
  无缝。页签下分隔线与轨迹工具栏边框均已移除，工具栏改为两端淡入淡出的
  「霜柱」渐变背景；毛玻璃 blur：会话区 16px /
