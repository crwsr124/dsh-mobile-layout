...
- **极光背景**：四套预设（蓝紫 / 暖阳 / 青绿 / 单色·墨）——深色基底渐变之上
  是多层高饱和彩色光斑（26px 高斯模糊，光斑透明度 0.7-0.95），以 16s 周期
  漂移旋转缩放（幅度 4% + rotate ±3° + scale 1.04-1.16 + 透明度呼吸，肉眼
  可感知）；切换预设时光斑淡出-换色-淡入（250ms）；
...
释放布局空间），下滑回到**底部 8px 内**自动滑回。防抖与优雅性细节：

- 底部 8~64px 的**滞回带**：小幅抖动不会反复隐藏/弹出；
- 输入框有**焦点**（正在输入）时不收起；
- 滑出动画完成后才 `display: none` 释放阅读空间；回到底部先恢复布局再滑入；
- `prefers-reduced-motion` 用户自动关闭动画；
- 只作用于手机视口（<1024px），桌面端完全不受影响；
- 与 app 自带「回到底部」按钮天然配合：点它滚到底 → 输入框自动滑回。
...
- **轨迹视图分隔线柔化**：轨迹工具栏（sticky 底边）、迷你时间线绘图区底边、
  标签列的实线边框（主题 token 全对比度灰线）改为 10-12% 半透明细缝；
- **会话头部防重叠**：标题行允许换行（`flex-wrap`），窄列（打开轨迹/小窗口）
  时「标准模式 / 3 个后台任务 / Session log」不再互相挤压；Session log 按钮
  收紧内边距与字号；
- **自定义图片不再糊**：玻璃模式下自定义壁纸的会话区模糊 16px→3px、侧栏
  26px→8px（`data-dml-bg="custom"` 门控），图片清晰可见；表面不透明度微增
  6% 保证无模糊时的文字可读性。
