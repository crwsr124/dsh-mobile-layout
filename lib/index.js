 ...
 * The upload target is dynamic (the directory the client is browsing; the
 * configured `defaultPath` when absent), never a hardcoded location. The
 * file name is a sanitized single segment, the size is capped and conflicts
...
export function apply(ctx, config) {
	...
	/** Resolve the upload target directory: the client-supplied `dir` when
	 *  present, else the configured `defaultPath`; the resolved realpath must
	 *  be an existing directory inside the roots allowlist (symlink-safe).
	 *  Returns the realpath, or null with the failure already answered. */
	const resolveUploadDir = async (req, res, u) => {
		const dirParam = u.searchParams.get("dir") ?? "";
		const rawDir = dirParam || defaultPath || "";
	...
	const handleUpload = async (req, res, u) => {
		const dir = await resolveUploadDir(req, res, u);
		if (!dir) return;
