 ...
 *   POST /mobile-files/upload?name=<n>&dir=   — raw-body upload into `dir`
 *
 ...
 * `roots` allowlist entries — reads and the upload target directory alike.
 * The upload target is dynamic: the directory the client is browsing wins;
 * without one it falls back to the configured `uploadDir`, else to an
 * auto-created `<defaultPath>/upload` — never the root itself. The file name
 * is a sanitized single segment, the size is capped and conflicts auto-rename
 * — no path traversal, no overwrite, no read-back. No-cache.
 *
...
export function apply(ctx, config) {
	...
	// fallback upload target (used only when the client sends no `dir`):
	// explicit `uploadDir` wins, else an auto-created <defaultPath>/upload.
	const uploadDirCfg = typeof config?.uploadDir === "string" && config.uploadDir ? config.uploadDir : null;
	...
	/** Resolve the client-supplied `dir` param: must be an existing directory
	 *  whose realpath stays inside the roots allowlist (symlink-safe).
	 *  Returns the realpath, or null with the failure already answered. */
	const resolveDirParam = async (req, res, u) => {
		const dirParam = u.searchParams.get("dir") ?? "";
		const rawDir = dirParam || "";
		if (!rawDir || !rawDir.startsWith("/") || rawDir.includes("\0")) {
			sendJson(res, 400, { error: "bad-dir", message: "dir must be an absolute path inside the roots allowlist" });
			return null;
		}
		let real;
		try {
			if (!(await stat(rawDir)).isDirectory()) throw new Error("not-a-directory");
			real = await realpath(rawDir);
		} catch {
			sendJson(res, 400, { error: "bad-dir", message: "dir does not exist or is not a directory" });
			return null;
		}
		if (!(await matchingRoot(real))) {
			sendJson(res, 403, { error: "forbidden", message: "dir is outside the roots allowlist" });
			return null;
		}
		return real;
	};

	/** Resolve the fallback upload directory when the client sends no `dir`:
	 *  `uploadDir` config, else <defaultPath>/upload — creating missing levels,
	 *  with the nearest existing ancestor and the final realpath both required
	 *  to stay rooted (symlink-safe). Returns the realpath, or null when there
	 *  is nothing valid to fall back to (fail-closed, never the root). */
	const resolveFallbackDir = async () => {
		if (!defaultPath && !uploadDirCfg) return null;
		const abs = resolve(uploadDirCfg || join(defaultPath, "upload"));
		let probe = abs;
		for (;;) {
			try {
	...
	/** POST /mobile-files/upload?name=<file>[&dir=<abs dir>] — stream the raw
	 *  request body into <dir>/<sanitized name>; auto-rename on conflicts;
	 *  enforce the size cap (content-length fast path + streaming count);
	 *  partial writes are unlinked on failure. */
	const handleUpload = async (req, res, u) => {
		let dir = null;
		if (u.searchParams.get("dir") ?? "") {
			dir = await resolveDirParam(req, res, u);
			if (!dir) return;
		} else {
			dir = await resolveFallbackDir();
			if (!dir) {
