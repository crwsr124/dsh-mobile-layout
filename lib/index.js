 ...
 *   GET  /mobile-files/read?path=<abs>        — inline file preview
 *   GET  /mobile-files/download?path=<abs>    — streamed attachment download
 *   POST /mobile-files/upload?name=<n>        — raw-body upload into the upload dir
 *
 ...
 * Uploads land in a dedicated directory only: the configured `uploadDir`,
 * else an auto-created `<defaultPath>/upload` — never the root. The file
...
export function apply(ctx, config) {
	...
	/** Resolve the upload directory: `uploadDir` config, else an auto-created
	 *  <defaultPath>/upload — creating missing levels, with the nearest
	 *  existing ancestor and the final realpath both required to stay rooted
	 *  (symlink-safe). Returns the realpath, or null when there is nothing
	 *  valid to upload into (fail-closed, never the root). */
	 ...
	 *  into the upload directory (uploadDir config or <defaultPath>/upload —
	 *  never the root); auto-rename on conflicts; enforce the size cap
	 *  (content-length fast path + streaming count); partial writes are
	 *  unlinked on failure. */
		...
		handler: async (req, res) => {
			...
			try {
				...
				if (action === "read" || action === "download") {
					...
					const headers = {
						"content-type": downloading ? "application/octet-stream" : text ? `${mimeOf(rawPath)}; charset=utf-8` : mimeOf(rawPath),
						"content-length": String(st.size),
						"cache-control": "no-store",
						"content-disposition": `${downloading ? "attachment" : "inline"}; filename*=UTF-8''${encodeURIComponent(basename(rawPath))}`
					};
					...
					if (downloading) {
						...
						res.end(await readFile(rawPath));
					}
					return;
				}
